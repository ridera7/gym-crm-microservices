...
@Component
public class AuthorizationGatewayFilterFactory implements GatewayFilterFactory<AuthorizationGatewayFilterFactory.Config> {

    private static final List<String> EXCLUDED_URLS = List.of(
            "/authentication-service/api/v1/login",
            "/authentication-service/api/v1/validate-token");
    private final WebClient.Builder webClientBuilder;

    public AuthorizationGatewayFilterFactory(WebClient.Builder webClientBuilder) {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            return webClientBuilder.build()
                    .get()
                    .uri("lb://AUTHENTICATION-SERVICE/api/v1/validate-token")
